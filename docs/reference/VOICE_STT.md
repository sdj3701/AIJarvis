...
답변 TTS가 재생되는 동안 마이크는 로컬 Vosk 인식을 사용해 호출어를 감시한다. 기존처럼
최종 문장이 끝날 때까지 기다리지 않고 부분 또는 최종 결과에 `자비스`가 나타나면 즉시
중단한다. 단, 최근 1초 안에 `voice.stt.command.speech_threshold_dbfs` 이상의 입력이
있어야 한다. 현재 읽는 답변 자체에 `자비스`가 포함된 경우에는 스피커 소리 오인을 막기
위해 부분 결과를 사용하지 않고 정확한 최종 호출만 인정한다.

사용 순서:

1. 답변 중 `[끼어들기 감시]`가 표시되는지 확인한다.
2. 마이크 가까이에서 “자비스”를 짧고 또렷하게 말한다.
3. `[말 끼어들기]`와 `[답변 중단]`이 표시되며 현재 SAPI 재생이 종료된다.
4. “무엇을 도와드릴까요.”가 들리면 새 질문을 말한다.

```text
[끼어들기 감시] 답변 중 '자비스'라고 부르면 중단합니다.
[끼어들기 마이크] -30.2 dBFS · 감시 중
[끼어들기 STT 부분] 자비스
[말 끼어들기] '자비스'를 감지했습니다.
[답변 중단] 새 질문을 받을 준비를 합니다.
```

`[끼어들기 마이크]`만 보이고 STT 문구가 없다면 말할 때 dBFS가 `-42`보다 큰지 먼저
확인한다. 예를 들어 `-30`은 통과하지만 `-55`는 통과하지 않는다. 발화가 계속 `-42`
아래라면 Windows 마이크 입력 볼륨이나 거리를 먼저 조정한다. 그래도 작을 때만
`D:\Jarvis\config\settings.yaml`의 `speech_threshold_dbfs`를 `-48`처럼 조금 낮춘다.
값을 너무 낮추면 스피커 소리와 방 소음도 음성으로 판단할 수 있다. 설정 변경 뒤에는
프로그램을 `Ctrl+C`로 종료하고 다시 실행해야 한다.
...
- 읽는 답변 속 `자비스` 부분 결과를 사용자 끼어들기로 오인하지 않는 회귀 테스트 통과
