...
- Vosk 부분 결과는 화면 진단에만 쓰고 호출은 `자비스` 최종 결과에서만 확정한다.
...
답변 TTS가 재생되는 동안 마이크는 로컬 Vosk 전체 문장 인식을 사용해 호출어를 감시한다.
스피커에서 나온 답변 자체를 사용자 음성으로 오인하지 않도록 최종 인식 결과가 정확히
`자비스`일 때만 중단한다.

사용 순서:

1. 답변 중 `[끼어들기 감시]`가 표시되는지 확인한다.
2. “자비스”만 짧게 말하고 잠시 멈춘다.
