...
사용자 질문 인입 시 **① 로컬 RAG 지식베이스를 우선 초고속 조회**하고, 지식이 부족할 때만 **② Ollama(로컬 LLM) 심층 추론 및 웹 검색으로 확장**한다. 새로 도출된 질의응답과 중요 데이터는 정리하여 **③ 로컬 RAG 저장소에 자동 보관·누적**함으로써 자가 성장형 지식 체계를 구축한다. 외부 텍스트는 항상 신뢰할 수 없는 데이터로 취급하고, 핵심 주장에 출처와 확인 날짜를 연결한다. 이 Phase 완료가 MVP 경계다.
...
### P3-04 문서 검색 및 RAG 선조회 라우팅

1. 질의 인입 시 **로컬 지식베이스/RAG FTS를 1차 선조회**하여 충분한 근거가 있는지 확인한다.
2. 로컬 지식으로 완결 가능한 경우 외부 검색이나 무거운 LLM 심층 추론을 생략하고 즉시 정확한 근거와 함께 답변한다.
3. 로컬 지식이 부족하거나 부재할 경우에만 2단계(Ollama/웹 검색)로 질의를 확장한다.
4. 같은 문서에서 지나치게 겹치는 chunk를 합치고 최대 chunk 수와 토큰 예산을 적용한다.
5. 각 결과에 파일명, 상대 경로, chunk ID, 문자 범위, transfer class를 포함한다.
6. `local_only` 본문은 외부 LLM 프롬프트에 넣지 않는다. 이 경우 로컬 검색 결과의 존재만 알리고 정책에 따라 사용자에게 선택지를 준다.
