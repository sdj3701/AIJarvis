# Phase 3 — 로컬 하이브리드 RAG (FTS5 + 벡터 임베딩 + RRF)

## 1. 목표

외부 유료 API에 의존하지 않고, 내 PC에서 100% 동작하는 **<로컬 하이브리드 RAG (Hybrid Search)>** 엔진을 구현한다.
정확한 키워드/코드를 잡는 **① SQLite FTS5 (BM25)**와 동의어/문맥을 잡는 **② 로컬 오픈소스 벡터 임베딩**을 결합하고, **③ RRF (Reciprocal Rank Fusion) 상호 순위 결합 알고리즘**으로 최종 랭킹을 산출한다.

사용자 질문 인입 시 **1단계: 하이브리드 RAG 지식베이스 선조회(0.01초)**로 근거를 찾고, 지식이 부족할 때만 **2단계: 로컬 Ollama 모델 심층 추론**으로 확장한다. 새로 도출된 답변은 **3단계: 로컬 RAG 저장소에 자동 보관·누적**함으로써 자가 성장형 지식 체계를 구축한다.

> **💡 핵심 원칙**: 키워드의 정확성과 벡터의 문맥 이해력을 융합한 **하이브리드 검색(Hybrid RAG)**을 순수 로컬 오프라인(비용 $0.00)으로 완결한다.
...
### P3-04 하이브리드 검색과 RRF 랭킹 융합 (Hybrid Search)

1. **하이브리드 후보 수집**:
   - 키워드 경로: SQLite FTS5 (BM25)로 정확한 키워드/코드/고유명사 후보 수집
   - 벡터 경로: 로컬 오픈소스 임베딩으로 문맥/동의어/의미적 유사 후보 수집
2. **RRF (Reciprocal Rank Fusion) 랭킹 융합**:
   $$RRF\_Score(d) = \sum_{m \in \{FTS, Vector\}} \frac{1}{k + rank_m(d)} \quad (k=60)$$
   두 경로의 순위를 수학적으로 결합하여 단어 불일치와 고유명사 오류를 동시에 방지한다.
3. **1차 완결 라우팅**: 하이브리드 RAG 지식으로 완결 가능하면 즉시 정확한 근거와 함께 응답.
4. **2단계 확장**: 지식이 부족하거나 없을 때만 Ollama 심층 모델 및 웹 검색으로 확장.
5. 같은 문서에서 지나치게 겹치는 chunk를 병합하고 토큰 예산을 적용한다.
6. `local_only` 본문은 외부 LLM 프롬프트에 넣지 않고 로컬에서만 완결 처리한다.
...
- 검색 실패와 “결과 0건”을 구분한다.
- **지식 자동 누적(Knowledge Distillation)**: 새롭게 검색·도출된 질의응답 및 중요 정보는 세션 종료 또는 턴 완료 시 로컬 RAG 저장소(`docs/notes/` 및 SQLite)에 요약 정리되어 자동 색인(Index)되며, 차후 동일/유사 질문 시 1단계 RAG에서 즉시 활용된다.
