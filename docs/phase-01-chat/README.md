...
- [x] 멀티턴 문맥 유지
- [x] Ollama 오류가 사용자 메시지와 이벤트에 기록됨
- [x] 20회 강제 종료에서 사용자 입력 유실 0건
- [x] 무한 재시도 없음
- [x] 컨텍스트 압축 후 최근 턴 유지
- [x] 일/월 예산 100%에서 신규 요청 차단
- [x] 실제 로컬 모델 수동 스모크 3턴 성공, GPU 적재, 비용 USD 0.00 확인
- [x] Ollama loopback 외 외부 HTTP 0건

완료 증거: [phase1-20260811.json](../../artifacts/gates/phase1-20260811.json) — 283 passed,
5개 종료 지점 × 4회 입력 유실 0건. 실제 `qwen3.5:9b` 3턴 한국어 대화와 16K GPU 적재도 별도로 확인했다.
