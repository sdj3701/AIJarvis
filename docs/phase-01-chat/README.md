...
제공자 API로 멀티턴 대화를 수행하고, 사용자 입력을 API 호출 전에 저장하며, 재시도·비용·컨텍스트 한도를 통제한다.
...
- `settings.yaml`의 provider, model, pricing 값이 placeholder가 아니다.
- 해당 API 키가 Windows Credential Manager 또는 개발용 `.env`에 있다.

## 3. 만들 파일

```text
app/budget.py
app/llm/{base.py,<provider>_client.py,fake.py}
...
저장보다 API 호출이 먼저 일어나는 구현은 허용하지 않는다.
...
1. `wiring.py`에서 선택된 제공자 구현 하나만 조립한다.
2. timeout·429·5xx만 지수 backoff+jitter로 최대 횟수까지 재시도한다.
3. 인증 오류·잘못된 요청은 재시도하지 않는다.
4. 각 시도에 `llm.request`, `llm.retry`, `llm.response` 이벤트를 남긴다.
5. 제공자 응답을 내부 타입으로 변환하고 알 수 없는 finish reason은 거부한다.

제공자 SDK 타입이 `orchestrator`나 `memory`로 새어나오면 안 된다.
...
5. 입력 자체가 한도를 넘으면 API를 호출하지 않고 사용자 오류로 끝낸다.
...
- [ ] 멀티턴 문맥 유지
- [ ] API 오류가 사용자 메시지와 이벤트에 기록됨
- [ ] 20회 강제 종료에서 사용자 입력 유실 0건
- [ ] 무한 재시도 없음
- [ ] 컨텍스트 압축 후 최근 턴 유지
- [ ] 일/월 예산 100%에서 신규 요청 차단
- [ ] 실제 API 수동 스모크 3턴 성공 및 비용 기록 확인
