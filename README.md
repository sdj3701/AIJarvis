...
읽는 중에는 터미널에 `[끼어들기 감시]`가 표시된다. 중간에 멈추려면 **“자비스”만 짧게
말한 뒤 잠시 기다린다.** `[답변 중단]`과 “무엇을 도와드릴까요.”가 들리면 새 질문을
말하면 된다. 스피커 답변 속의 단어를 사용자 호출로 오인하지 않도록 “자비스 프로젝트”
같은 긴 문장은 끼어들기 호출로 인정하지 않는다.
