...
답변은 `Microsoft Heami Desktop`의 기본 속도보다 실측 약 1.55배 빠르게 읽는다. 답변을
읽는 중에는 `[끼어들기 감시]`, `[끼어들기 마이크]`, `[끼어들기 STT 부분]`이 표시된다.
중간에 멈추려면 **마이크 가까이에서 “자비스”라고 짧고 또렷하게 말한다.** 부분 인식에서
호출어가 확인되는 즉시 답변을 끊는다. TTS 기준선보다 급상승한 근거리 음성이면서
WebRTC VAD가 사람 음성으로 본 구간만 STT에 전달한다. `[답변 중단]`과
“무엇을 도와드릴까요.”가 들리면 새 질문을 말하면 된다. 현재 읽는 답변 자체에
“자비스”가 들어 있으면 스피커 소리의 부분·긴 최종 인식만으로는 중단하지 않는다.
